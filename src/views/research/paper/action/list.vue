  ...
  <div style="padding:30px;">
    <el-form ref="searchObj" :inline="true" :model="searchObj" class="demo-form-inline">
      <el-form-item prop="paperName" label="论文题目">
        <el-input v-model="searchObj.paperName" placeholder="论文名称" />
      </el-form-item>
      <el-form-item prop="periodicalName" label="期刊名称">
        <el-input v-model="searchObj.periodicalName" placeholder="期刊名称" />
      </el-form-item>
    ...
    <el-table :data="list" stripe border fit highlight-current-row style="width: 100%">
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button type="warning" size="mini" icon="el-icon-edit" @click="editPaper(scope.row.paperId)" />
          <el-button type="danger" size="mini" icon="el-icon-delete" @click="removePaper(scope.row.paperId)" />
        </template>
      </el-table-column>
